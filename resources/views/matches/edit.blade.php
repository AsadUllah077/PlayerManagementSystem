<!-- resources/views/matches/edit.blade.php -->
@extends('layouts.app')
...
<div class="container">
    ...
    <form action="{{ route('matches.update', $match->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="tournament_id" name="tournament_id" required>
                ...
                @foreach($tournaments as $tournament)
                    <option value="{{ $tournament->id }}" {{ $match->tournament_id == $tournament->id ? 'selected' : '' }}>{{ $tournament->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" class="form-control" id="match_date" name="match_date" value="{{ $match->match_date }}">
        </div>
        <div class="form-group">
            <label for="location">Location</label>
            <input type="text" class="form-control" id="location" name="location" value="{{ $match->location }}">
        </div>
